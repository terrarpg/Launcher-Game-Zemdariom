    ...
    accounts() {
        document.querySelector('.accounts-list').addEventListener('click', async e => {
            ...
            try {
                ...
                // Si l'utilisateur clique sur l'élément "add", on redirige vers la page de connexion
                if (id == 'add') {
                    document.querySelector('.cancel-home').style.display = 'inline'
                    return changePanel('login')
                }
                
                // Gestion du clic sur un compte existant
                if (e.target.classList.contains('account') || 
                    e.target.closest('.account') && !e.target.classList.contains("delete-profile")) {
                    
                    // Récupérer l'ID du compte depuis l'élément le plus proche
                    const accountElement = e.target.closest('.account');
                    const accountId = accountElement ? accountElement.id : id;
                    
                    // Si c'est l'élément "add", on ne fait rien de plus
                    ...
                    let account = await this.db.readData('accounts', accountId);
                    
                    // Ouvrir une nouvelle fenêtre avec l'URL spécifiée
                    const username = encodeURIComponent(account.name || account.username || '');
                    const url = `https://serv-zendariom.azuriom.cloud/skin-api?user=${username}`;
                    
                    // Fermer le popup avant d'ouvrir la fenêtre
